import os, ast, time, math, shutil, glob
import pandas as pd
...
def adding_aatype(csv, before_pdb_dir, pdb_output_dir):
    ...
    df = pd.read_csv(csv)
    print(f"CSV entries: {df.shape[0]}")
    os.makedirs(pdb_output_dir, exist_ok=True)

    # Get the set of existing PDB filenames in before_pdb_dir
    existing_pdbs = set(
        os.path.basename(p) for p in glob.glob(f"{before_pdb_dir}/*.pdb")
    )
    print(f"PDB files found in before_pdb_dir: {len(existing_pdbs)}")

    count = 0
    for idx, row in tqdm(df.iterrows(), total=len(df)):
        pdb_name = row["link_name"]
        print(f"Processing pdb_name: {pdb_name}")

        # Make sure the file exists in before_pdb_dir
        if pdb_name not in existing_pdbs:
            print(f"Skipping {pdb_name}: file not found in before_pdb_dir")
            continue

        full_pdb_path = os.path.join(before_pdb_dir, pdb_name)
        print(f"Full PDB path: {full_pdb_path}")

        if not os.path.exists(full_pdb_path):
            print(f"Warning: file not found: {full_pdb_path}")
            continue
        ...
        # Parse sequence_dict safely
        sequence = ast.literal_eval(row["sequence_dict"])

        # Support chain A only, or both A and B
        seq_A = sequence.get("A", "")
        seq_B = sequence.get("B", "")

        # Detect which binder chains are expected from sequence_dict
        binder_chains = []
        if seq_A:
            binder_chains.append("A")
        if seq_B:
            binder_chains.append("B")

        if not binder_chains:
            print(
                f"Warning: no valid binder chain (A/B) found in sequence_dict for {pdb_name}"
            )
            continue

        # Collect residue indices separately for each chain
        chain_res_indices = {chain: [] for chain in binder_chains}
        chain_last_resi = {chain: None for chain in binder_chains}
        ...
        for line in pdb_lines:
            if line.startswith("ATOM"):
                ...
                if chain_id in binder_chains:
                    resi = line[22:26]
                    if resi != chain_last_resi[chain_id]:
                        chain_res_indices[chain_id].append(resi)
                        chain_last_resi[chain_id] = resi

        # Validate residue counts per chain
        mismatch = False
        if "A" in binder_chains and len(chain_res_indices["A"]) != len(seq_A):
            print(
                f"Warning: residue count mismatch in chain A for {pdb_name}, "
                f"pdb={len(chain_res_indices['A'])}, seq={len(seq_A)}"
            )
            mismatch = True

        if "B" in binder_chains and len(chain_res_indices["B"]) != len(seq_B):
            print(
                f"Warning: residue count mismatch in chain B for {pdb_name}, "
                f"pdb={len(chain_res_indices['B'])}, seq={len(seq_B)}"
            )
            mismatch = True
        ...
        # Build mapping: (chain_id, resi) -> residue name
        resi_to_resname = {}

        if "A" in binder_chains:
            for resi, aa in zip(chain_res_indices["A"], seq_A):
                resi_to_resname[("A", resi)] = one_to_three.get(aa, "UNK")

        if "B" in binder_chains:
            for resi, aa in zip(chain_res_indices["B"], seq_B):
                resi_to_resname[("B", resi)] = one_to_three.get(aa, "UNK")
        ...
        # Output path with seq_idx
        output_pdb = os.path.join(
            pdb_output_dir,
            os.path.basename(row["link_name"]).replace(
                ".pdb", f"_{row['seq_idx']}.pdb"
            ),
        )
    ...
    print(f"Total generated files: {count}")
...
class Sampler(object):
    def __init__(self, config, use_gt_masks=False, ddp=False):
        ...
        self.train_loader, self.test_loader, _, _ = get_dataloader(
            self.config, ddp=ddp, sample=True
        )
    ...
    def sample(self, ts, save_dir, save_traj=False, inpaint=""):
        ...
        if self.config.conf_ckpt is not None:
            ...
            self.conf_model = Confidence(
                EquiformerV2(**conf_ckpt["config"].model), conf_ckpt["config"]
            ).cuda()
        ...
        with torch.no_grad():
            for batch in tqdm(self.test_loader):
                ...
                with torch.no_grad():
                    best_pred_idx, best_pred_rmsd, best_gt_idx, best_gt_rmsd = (
                        0,
                        999,
                        0,
                        999,
                    ...
                    for sample_idx in range(self.config.sample.n_samples):
                        ...
                        for i in range(batch_size):
                            pdb_path = sample_path.joinpath(
                                f"run_{sample_idx + 1}", f"{pdb_codes[i]}.pdb"
                            )
                        ...
                        pred_sc = self.model.decode(
                            batch, return_traj=save_traj, inpaint=inpaint
                        )
                        pred_sc = (
                            pred_sc - math.pi
                        ) * chi_mask  # shift torsions back to [-pi,pi]

                        if save_traj:
                            pred_sc_traj = pred_sc.clone()
                            pred_sc = pred_sc[-1]

                        all_atom_coords = self.idealizer(
                            aa_num, bb_coords, pred_sc
                        ) * atom_mask.unsqueeze(-1)
                        gt_idealized = self.idealizer(
                            aa_num, bb_coords, chi - math.pi
                        ) * atom_mask.unsqueeze(-1)
                        ...
                        for i in range(batch_size):
                            ...
                            mae, acc = metrics_per_chi(
                                pred_batch, chi_batch, chi_alt_batch, chi_mask_batch
                            )
                            ...
                            rmsd_idealized = atom_rmsd(
                                pred_pos_batch[:, 4:],
                                crds_batch_idealized[:, 4:],
                                atom_mask_batch[:, 4:],
                            )
                            core_rmsd = atom_rmsd(
                                pred_pos_batch[core][:, 4:],
                                crds_batch[core][:, 4:],
                                atom_mask_batch[core][:, 4:],
                            )
                            ...
                            if save_traj:
                                ...
                                for traj in pred_sc_traj:
                                    ...
                                    crds = self.idealizer(
                                        aa_batch, pred_pos_batch[:, :4], traj_batch
                            ...
                            if self.config.conf_ckpt is not None:
                                best_path = sample_path.joinpath(
                                    "best_run", f"{pdb_codes[i]}.pdb"
                                )
                                best_path.parent.mkdir(exist_ok=True, parents=True)

                                pred_rmsd, gt_rmsd = self.conf_model.get_pred(
                                    pred_sc, batch
                                )
                                ...
                                if best_pred_rmsd > pred_rmsd:
                                    ...
                                    output_dict[pdb_codes[i]]["best_pred_idx"] = (
                                        best_pred_idx + 1
                                    )
                                    output_dict[pdb_codes[i]]["best_gt_idx"] = (
                                        best_gt_idx + 1
                                    )
                                    output_dict[pdb_codes[i]]["best_pred_rmsd"] = (
                                        best_pred_rmsd
                                    )
                                    output_dict[pdb_codes[i]]["best_gt_rmsd"] = (
                                        best_gt_rmsd
                                    )
...
if __name__ == "__main__":
    ...
    after_pdb_dir_batch = os.path.join(
        args.save_dir,
        "after_pdbs_batch",
        f"{os.path.basename(before_pdb_dir)}",
    )
    ...
    after_pdbs = os.path.join(args.save_dir, "after_pdbs")
    os.makedirs(after_pdbs, exist_ok=True)

    sampler = Sampler(config, args.use_gt_masks)
    sampler.sample(
        time.strftime("%b%d-%H:%M:%S", time.gmtime()),
